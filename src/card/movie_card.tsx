...
export const MovieData:React.FC<MovieCardInterface> = ({movieid}) => {
  ...
  useEffect(() => {
		const datafetch = async () => {
			if (process.env.REACT_APP_APIKEY) {
				const data = await APIFetch(`/movie/${movieID}`, process.env.REACT_APP_APIKEY);
				setMovieData(data);
			} else {
  ...
  return (
    <article className="card">
      <img className="card__img" data-id = {moviedata?.id} src={imagesrc} alt={moviedata?.original_title} />
